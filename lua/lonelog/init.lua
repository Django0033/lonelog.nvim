...
function M.roll_dice(notation)
  ...
  if err then vim.notify("lonelog: " .. err, vim.log.levels.ERROR); return nil, err end
...
function M.roll_oracle(table_name)
  if table_name == "mythic" then
    ...
    vim.ui.input({ prompt = string.format("Chaos factor (1-9, default %d, +/- to change): ", chaos) }, function(input)
      if input == "+" then chaos = math.min(chaos + 1, 9)
      elseif input == "-" then chaos = math.max(chaos - 1, 1)
      else local n = tonumber(input); if n and n >= 1 and n <= 9 then chaos = n end end
...
function M.open_picker()
  M.ui.pick({
    title = "Lonelog",
    items = { { label = "Oracle", action = "oracle" }, { label = "Dice Roll", action = "dice" }, { label = "Navigate Tags", action = "tags" }, { label = "Navigate Scenes", action = "scenes" } },
    format_item = function(i) return i.label end,
    on_select = function(i)
      if i.action == "oracle" then
        M.ui.pick({ title = "Choose the Oracle", items = M.oracle.list_tables(), on_select = function(t) M.roll_oracle(t) end })
      elseif i.action == "dice" then
        vim.ui.input({ prompt = "Dice (e.g., 2d6+3): " }, function(n) if n and n ~= "" then M.roll_dice(n) end end)
      elseif i.action == "tags" then
        M.parsers.show_tags_picker()
      elseif i.action == "scenes" then
        M.parsers.show_scenes_picker()
      end
    end,
  })
end
