...
local tables = {
  ...
  binary = { name = "Binary Oracle", entries = {
...
local function weighted_random(entries)
  ...
  for _, entry in ipairs(entries) do
    ...
    if roll <= current then return entry end
  end
  return entries[#entries]
end

-- Get current chaos factor (for Mythic oracle)
function M.get_chaos() return chaos_factor end

-- Set chaos factor (must be 1-9)
function M.set_chaos(value) if value and value >= 1 and value <= 9 then chaos_factor = value; return true end return false end
...
function M.mythic_roll(chaos)
  ...
  if final <= 4 then result_val, display_val = "exceptional_no", "Exceptional No"
  elseif final <= 10 then result_val, display_val = "no", "No"
  elseif final <= 17 then result_val, display_val = "yes", "Yes"
  else result_val, display_val = "exceptional_yes", "Exceptional Yes" end
  return { table = "mythic", table_name = "Mythic", value = result_val, display = display_val, chaos = chaos, chaos_modifier = chaos_mod, d10 = { d10_1, d10_2 }, dice_total = d10_1 + d10_2, final = final }
end
...
function M.roll(table_name)
  ...
  if table_name == "mythic" then return M.mythic_roll(chaos_factor) end
  if not tables[table_name] then return nil, "Unknown oracle table: " .. table_name end
  local result = weighted_random(tables[table_name].entries)
  return { table = table_name, table_name = tables[table_name].name, value = result.value, display = result.display, description = "" }
end
...
function M.get_table(name) return tables[name] and vim.deepcopy(tables[name]) or nil end
...
function M.format_result(result)
  ...
  if result.table == "mythic" then
    ...
    return string.format("[%s] (2d10: %d + caos(%s) = %d) %s", result.table_name, result.dice_total, cs, result.final, result.display)
  end
