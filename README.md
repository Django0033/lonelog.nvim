...
- **Dungeon status** — `=== Dungeon Status ===` block with auto-collected room tags and ASCII exit map, insert or update at `<leader>lK`
...
| `:LonelogDungeonStatus` | Insert/update `=== Dungeon Status ===` block with room tags and ASCII exit map |
